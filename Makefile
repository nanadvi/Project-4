...
all: $(BINDIR)/vm 
apps: $(BINDIR)/hello.so $(BINDIR)/sleep.so $(BINDIR)/file.so $(BINDIR)/file2.so $(BINDIR)/thread.so $(BINDIR)/mutex.so $(BINDIR)/memory.so $(BINDIR)/preempt.so $(BINDIR)/badprogram.so $(BINDIR)/shell.so $(BINDIR)/badprogram2.so $(BINDIR)/copyfile.so
